...
const TYPE_LABEL: Record<string, string> = {
  online: '在线充值', stripe: 'Stripe', creem: 'Creem', waffo: 'Waffo', waffo_pancake: 'Waffo Pancake',
}
      ...
      <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white shadow-xl">
        <div className="p-6 sm:p-8">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="button" onClick={() => { setShowTopUp(true); setSelectedMethod(payMethods[0] || '') }}
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-1 rounded-lg bg-gray-100 p-1">
            {(['all', 'topup', 'consume'] as const).map(([key, label]) => (
              <button key={key} type="button" onClick={() => { setFilter(key as typeof filter); setPage(1) }}
            ...
            <tbody className="divide-y divide-gray-50">
              {historyQuery.isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <tr key={tx.id} className="transition-colors hover:bg-gray-50">
                  ...
                  <td className="py-3 pr-4 text-gray-700">{TYPE_LABEL[tx.payment_provider] || tx.payment_provider || tx.payment_method}</td>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-sm animate-fade-in-up rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
            ...
            <div className="mt-3 relative">
              ...
              <input type="number" value={topUpAmount} onChange={(e) => setTopUpAmount(e.target.value)} placeholder="自定义金额"
                ...
                <div className="flex flex-wrap gap-2">
                  {payMethods.map((m) => (
                    <button key={m} type="button" onClick={() => setSelectedMethod(m)}
                      className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${selectedMethod === m ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}>{m}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button type="button" onClick={handleTopUp} disabled={!topUpAmount || Number(topUpAmount) <= 0 || payMutation.isPending}
