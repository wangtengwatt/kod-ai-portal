...
### v0.3.0 (2026-08-04) —— 蒜宝官网版

- 蒜宝官网功能完善：看板、钱包、充值、中转站配置等核心模块上线
- 后端：新增钱包与充值模块（充值下单、支付回调、订单管理、钱包查询）
- 后端：新增会话管理（Session 列表与密钥查询）
- 后端：新增日志同步模块（API 请求日志记录与轮询同步）
- 后端：中转站配置优化（客户端密钥自动化配置、URL 后端适配）
- 后端：CORS 跨域配置完善
- 后端：数据库建表脚本更新（看板小时表、模型摘要表、订单表、API 日志表）
- 前端：新增数据看板页面（`/dashboard`，含用量曲线、模型分布、配额统计）
- 前端：新增钱包充值页面（`/wallet`，含充值记录、套餐选择、支付流程）
- 前端：导航栏新增看板与钱包入口
- 文档：新增 PRD 文档、看板表结构文档、日志同步轮询文档、接口文档
